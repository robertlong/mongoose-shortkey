...
exports.init = function(database, opts) {
	...
	var collectionName = opts.collection || 'shortkeys',
	...
	ShortKey = db.model(collectionName, schema);
};
...
exports.plugin = function(schema, options) {
	...
	if (!options.category) throw new Error('ShortKey category required');
	...
	ShortKey.findOne(query, function (err, res) {
		if (!res)
			(new ShortKey({
	...
	schema.pre('save', function (next) {
		...
		function save () {
			...
			if (ready)
				ShortKey.collection.findAndModify(query, [], {
					$inc: {
						counter: 1
					}
				}, {
